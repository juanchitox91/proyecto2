        ...
        public static List<Accion> getAccionesPorRoles(long idusuario)
        {
            ...
            try
            {
                ...
                sql = $"select a.id, a.nombre, a.descripcion, case when " +
                    ...
                    $" > 0 then 'SI' else 'NO' end ACTIVO, a.parent_id from dbo.accion a where a.parent_id <> 0 order by 1,5";
                ...
                while (dataReader.Read())
                {
                    acciones.Add(
                             ...
                             Parent_id = Convert.ToInt64(dataReader.GetValue(4).ToString()),
                         });
                }
                command.Dispose(); cnn.Close();
            }
        ...
        public static Dictionary<string, string> GetPermisosOperaciones(string id_rol)
        {
            ...
            try
            {
                ...
                sql = $"select ac.id id_padre, ac.nombre nombre_padre, case when " +
                    $"(select count(*) from (select a.id, a.parent_id, a.nombre, a.descripcion, case when  (select count(1) from  dbo.rol r " +
                    $"where (exists (select id from dbo.accionrol where idrol = r.id and activo) and  r.id ={id_rol}) or (select count(1) from dbo.rol r  where r.id = {id_rol} and r.esadministrador = 1) > 0) " +
